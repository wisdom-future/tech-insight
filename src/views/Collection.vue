  ...
  <div class="space-y-6">
    ...
    <div class="bg-white shadow rounded-lg p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <!-- 学术论文卡片 -->
        <DataCard
          :data="techData.academicPapers"
          color="blue"
          :loading="loading"
        />
        <!-- 专利数据卡片 -->
        <DataCard
          :data="techData.patentData"
          color="green"
          :loading="loading"
        />
        <!-- 开源项目卡片 -->
        <DataCard
          :data="techData.openSourceProjects"
          color="purple"
          :loading="loading"
        />
        <!-- 技术新闻卡片 -->
        <DataCard
          :data="techData.techNews"
          color="orange"
          :loading="loading"
        />
      </div>
    ...
    <div class="bg-white shadow rounded-lg p-6">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
        <!-- 标杆动态卡片 -->
        <DataCard
          :data="benchmarkData.industryDynamics"
          color="indigo"
          :loading="loading"
        />
        <!-- 技术创新卡片 -->
        <DataCard
          :data="benchmarkData.techInnovation"
          color="pink"
          :loading="loading"
        />
        <!-- 产品发布卡片 -->
        <DataCard
          :data="benchmarkData.productLaunches"
          color="cyan"
          :loading="loading"
        />
        <!-- 人才流动卡片 -->
        <DataCard
          :data="benchmarkData.talentMovement"
          color="yellow"
          :loading="loading"
        />
      </div>
...
const fetchData = async () => {
  ...
  try {
    ...
    if (response.success) {
      ...
      lastUpdate.value = dayjs(response.lastUpdated).format('YYYY-MM-DD HH:mm:ss')
      console.log('数据更新完成')
